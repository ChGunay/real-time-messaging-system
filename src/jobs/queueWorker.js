...
class QueueWorker {
  ...
  async processReadyMessages() {
    ...
    try {
      ...
      if (successCount > 0) {
        logger.info(`Queue worker completed: ${successCount} messages queued successfully, ${errorCount} errors`);
      }
  ...
  async queueMessage(autoMessage) {
    try {
      ...
      if (!autoMessage.sender || !autoMessage.receiver || !autoMessage.content) {
        throw new Error('Invalid message data: missing sender, receiver, or content');
      }
      ...
      logger.debug(`Message queued: ${autoMessage._id} from ${autoMessage.sender.username} to ${autoMessage.receiver.username}`);
  ...
  async handleMessageError(autoMessage, errorMessage) {
    try {
      ...
      if (!autoMessage.canRetry()) {
        logger.error(`Message ${autoMessage._id} failed permanently after ${autoMessage.retryCount} attempts`);
      }
    } catch (updateError) {
      logger.error(`Error updating message ${autoMessage._id} with error:`, updateError);
    }
  }
  ...
  async getDetailedStats() {
    try {
      const [readyCount, queuedCount, failedCount, rabbitMQHealth] = await Promise.all([
      ...
      return {
        ...
        averageProcessingTime: this.stats.totalRuns > 0 
