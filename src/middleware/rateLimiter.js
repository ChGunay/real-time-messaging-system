...
const createRedisStore = () => {
  try {
    ...
    if (!redisConnection.client || !redisConnection.client.isReady) {
      logger.warn('Redis not connected yet, using memory store for rate limiting');
      return undefined; 
    }
    ...
    logger.warn('Redis store for rate limiting not available, using memory store:', error.message);
    return undefined; 
  }
};
...
const socketRateLimiter = {
  ...
  checkMessageRate: (userId, maxMessages = 10, windowMs = 60000) => {
    ...
    const validMessages = userMessages.filter(timestamp => now - timestamp < windowMs);
