...
const handleRabbitMQError = (err) => {
  ...
  return new AppError('Message queue service temporarily unavailable', 503, 'QUEUE_ERROR');
};
...
const globalErrorHandler = (err, req, res, next) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    if (error.name === 'AMQPConnectionError') error = handleRabbitMQError(error);
...
const handleDBConnectionError = (err) => {
  ...
  return new AppError('Database service temporarily unavailable', 503, 'DB_CONNECTION_ERROR');
};
...
const handleRateLimitError = (req, res) => {
  const err = new AppError('Too many requests from this IP, please try again later', 429, 'RATE_LIMIT_EXCEEDED');
