...
class RabbitMQConnection {
  ...
  async setupQueuesAndExchanges() {
    try {
      ...
      const messageSendingQueue = process.env.MESSAGE_SENDING_QUEUE || 'message_sending_queue';
  ...
  getConsumerChannel() {
    if (!this.isConnected || !this.consumerChannel) {
      throw new Error('RabbitMQ not connected or consumer channel not available');
    }
  ...
  getPublisherChannel() {
    if (!this.isConnected || !this.publisherChannel) {
      throw new Error('RabbitMQ not connected or publisher channel not available');
    }
